...
namespace SafeExamBrowser.UserInterface.Desktop
{
    public class UserInterfaceFactory : IUserInterfaceFactory
    {
        ...
        {
            return new Windows.AboutWindow(appConfig, text);
        }

        public IActionCenter CreateActionCenter()
        {
            return new Windows.ActionCenter();
        }
        ...
        {
            return Application.Current.Dispatcher.Invoke(() => 
                new Windows.LockScreen(message, title, settings, text, options));
        }

        public IWindow CreateLogWindow(ILogger logger)
        {
            var window = default(Windows.LogWindow);
            var windowReadyEvent = new AutoResetEvent(false);
            var windowThread = new Thread(() =>
            {
                window = new Windows.LogWindow(logger, text);
        ...
        {
            return Application.Current.Dispatcher.Invoke(() => 
                new Windows.PasswordDialog(message, title, text));
        }

        public IPasswordDialog CreatePasswordDialog(TextKey message, TextKey title)
        {
            return Application.Current.Dispatcher.Invoke(() => 
                new Windows.PasswordDialog(text.Get(message), text.Get(title), text));
        }
        ...
        {
            return Application.Current.Dispatcher.Invoke(() => 
                new Windows.ProctoringFinalizationDialog(text));
        }

        public IProctoringWindow CreateProctoringWindow(IProctoringControl control)
        {
            return Application.Current.Dispatcher.Invoke(() => 
                new Windows.ProctoringWindow(control));
        }
        ...
        {
            return Application.Current.Dispatcher.Invoke(() => 
                new Windows.RuntimeWindow(appConfig, text));
        }

        public IServerFailureDialog CreateServerFailureDialog(string info, bool showFallback)
        {
            return Application.Current.Dispatcher.Invoke(() => 
                new Windows.ServerFailureDialog(info, showFallback, text));
        }

        public ISplashScreen CreateSplashScreen(AppConfig appConfig = null)
        {
            var window = default(Windows.SplashScreen);
            var windowReadyEvent = new AutoResetEvent(false);
            var windowThread = new Thread(() =>
            {
                window = new Windows.SplashScreen(text, appConfig);
                window.Closed += (o, args) => window.Dispatcher.InvokeShutdown();
                window.Show();
        ...
        {
            return new Windows.Taskbar(logger);
        }

        public ITaskview CreateTaskview()
        {
            return new Windows.Taskview();
        }
        ...
        private void InitializeFontAwesome()
        {
            ...
            FontAwesome.WPF.ImageAwesome.CreateImageSource(FontAwesome.WPF.FontAwesomeIcon.Flag, Brushes.Black);
        }
    }
}
