...
namespace SafeExamBrowser.Monitoring.Keyboard
{
	public class KeyboardInterceptor : IKeyboardInterceptor
	{
		...
		// Nouvelle variable statique pour suivre l'état des blocages
		private static bool restrictionsEnabled = true;
		...
		private bool KeyboardHookCallback(int keyCode, KeyModifier modifier, KeyState state)
		{
			...
			// Détection de Ctrl+P pour basculer l'état des restrictions
			if (modifier.HasFlag(KeyModifier.Ctrl) && key == Key.P && state == KeyState.Pressed)
			{
				// Inverser l'état des restrictions
				restrictionsEnabled = !restrictionsEnabled;
				
				// Logguer le changement d'état
				if (restrictionsEnabled)
				{
					logger.Info("Restrictions have been ENABLED via Ctrl+P toggle.");
				}
				else
				{
					logger.Info("Restrictions have been DISABLED via Ctrl+P toggle.");
				}
				
				// Ne pas bloquer cette combinaison de touches
				return false;
			}
			
			// Si les restrictions sont désactivées, ne rien bloquer
			if (!restrictionsEnabled)
			{
				return false;
			}
			
			// Sinon, appliquer les règles de blocage normales
			block |= key == Key.Apps;
